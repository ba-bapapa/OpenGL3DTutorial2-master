...
*@param vsPath			頂点シェダーファイル名
*@param fsPath			フラグメントシェダーファイル名
*
*@retval true  初期化成功
*@retval false 初期化失敗
*/
bool SpriteRenderer::Init(size_t maxSpriteCount, const char* vsPath, const char* fsPath) {
	...
	indices.resize(maxSpriteCount * 6);//四角形ごとにインデックスは六個必要
	...
	if (!vao.Id() || !ibo.Id() || !vao.Id() || program->IsNull()){
		return false;
	}
